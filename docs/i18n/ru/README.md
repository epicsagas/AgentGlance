...
- GeekMagic SmallTV с одной из поддерживаемых прошивок (определяется автоматически и сохраняется при `--ip`):
  - **SD_RU / SD Pro** community-прошивка (ESP8266) — быстрая проверка — эта команда должна вернуть JSON с массивом `files`:

    ```bash
    curl -s http://<DEVICE_IP>/photo/list
    ```

  - **SmallTV Ultra стоковая прошивка** (ESP32, [GeekMagicClock/smalltv-ultra](https://github.com/GeekMagicClock/smalltv-ultra)) — быстрая проверка — эта команда должна вернуть JSON с ключом `theme`:

    ```bash
    curl -s http://<DEVICE_IP>/app.json
    ```

  Прочие стоковые варианты прошивки GeekMagic и ESP32 "PRO" используют *другой* API и **не** поддерживаются.
...
Автономная установка из этого репозитория — встроен одноимённый маркетплейс `agent-glance`; хаб не нужен.
...
| Grok Build | ✅ | ✅ | ✅ | файл хуков соответствует Claude-совместимой схеме; в рантайме не проверялся |
...
> [!WARNING]
> **Предупреждение о размере GIF**: Большие GIF-файлы создают высокую нагрузку на память устройства (RAM/Flash ESP8266) и могут вызывать неожиданные перезагрузки или сбои. Настоятельно рекомендуется держать размер файлов в пределах **< 100 KB**.
...
| Параметр | Режим `frame` | Режим `fullscreen` |
|---|---|---|
| **Оптимальное разрешение** | **224 × 116 px** (~1.93:1) или **116 × 116 px** (1:1) | **240 × 240 px** (квадрат 1:1) |
| **Область композиции** | Вписывается в `MIDDLE_BOX = (8, 46, 224, 116)` | Покрывает весь экран 1.54" SmallTV |
| **Рекомендуемый размер файла** | **< 100 KB** (Строгий макс < 300 KB во избежание сбоев RAM/OOM и перезагрузок ESP8266) |
| **Количество кадров** | **12 – 16 кадров** (рендерер автоматически уменьшает лишние кадры до `_MAX_FRAMES = 16`) |
| **Задержка кадра** | **80ms – 150ms** на кадр (цикл 1.2s – 2.0s) |
| **Палитра цветов** | **64 – 128 цветов** (оптимизирует скорость рендеринга и износ флеш-памяти) |

**Сжатие исходного GIF под спецификацию** (необработанные экспорты легко превышают несколько МБ): равномерно выбрать кадры по всему клипу, затем перекодировать с коротким целевым циклом, чтобы весь диапазон движения сохранился, даже если скорость воспроизведения сжата.

1 — выбрать ~14 кадров равномерно из исходника, обрезав/масштабировав по макету:

```bash
# режим frame: вписывается с леттербоксом в MIDDLE_BOX, поэтому достаточно уменьшить масштаб (обрезка не нужна)
ffmpeg -i source.gif -vf "select='not(mod(n,STEP))',scale=224:116:force_original_aspect_ratio=decrease" \
  -vsync 0 frames/f_%03d.png

# режим fullscreen: растягивается на весь экран 240x240, поэтому сначала обрежьте в квадрат, иначе будет искажение
ffmpeg -i source.gif -vf "select='not(mod(n,STEP))',scale=240:240:force_original_aspect_ratio=increase,crop=240:240" \
  -vsync 0 frames/f_%03d.png
```

`STEP` = количество кадров источника ÷ 14 (округление вниз) — узнать через ffprobe (`ffprobe -v error -select_streams v -show_entries stream=nb_frames -of default=nw=1 source.gif`).

2 — перекодировать выбранные кадры с коротким целевым циклом (10fps = 100мс/кадр ≈ 1.4с цикл для 14 кадров) и небольшой палитрой:
...
Всё ещё больше 300 KB? Сначала уменьшите `max_colors` до 32 (попробуйте также `dither=none`), прежде чем сокращать количество кадров — именно это на самом деле определяет стоимость цикла.
...
| `/agent-glance:theme` | Быстро взглянуть на собственные экраны устройства — погода, прогноз, часы (Ultra; монитор вернётся при следующей активности) |
...
## Справочник по API устройства (SmallTV Ultra, стоковая прошивка)

Темы: 1 погодные часы на сегодня · 2 прогноз · **3 фотоальбом** · 4–6 стили часов · 7 простые погодные часы.

| Action | Endpoint |
|---|---|
| загрузка изображения | `POST /doUpload?dir=/image/` (multipart-поле `image`; повторная загрузка с тем же именем перезаписывает) |
| закрепить на экране | `GET /set?img=/image/<f>` (URL-кодирование; требуется тема 3) |
| переключить тему | `GET /set?theme=<n>` |
| флаги тем | `GET /set?theme_list=0,0,1,0,0,0,0&sw_en=0&theme_interval=10` |
| удалить файл | `GET /delete?file=/image/<f>` |
| прочитать состояние | `GET /app.json` (`theme`), `/theme_list.json`, `/filelist?dir=/image/`, `/space.json` |

Подводные камни, найденные при зондировании реального устройства:

- Показанное изображение *закрепляется* через `/set?img=` — остальные файлы альбома остаются, но никогда не сменяются (нет флагов включения по фото, как в SD_RU; setup их не трогает).
- Анимированные GIF декодируются и зацикливаются локально; вся файловая система 3 МБ общая с ресурсами погоды/часов — держите GIF маленькими (~1 МБ свободно на стоковом устройстве).
- `/set?img=` и `/set?theme=` возвращают литеральный текст `OK`, а не JSON.
- ⚠️ та же модель доверия, что у SD_RU: все эндпоинты в LAN без аутентификации.
