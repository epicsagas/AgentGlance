...
У этой прошивки **нет текстового API**, так что "печатать" в неё нечего. Вместо этого скрипт рендерит GIF 240×240 с помощью Pillow и загружает его в фотоальбом устройства, делая это изображение единственным включённым фото, а Photo — единственной включённой темой, так что кадр остаётся неизменным вместо переключения на другую тему.
